...
`shared` a cross-boundary merge won by the dependency. The merged environment cannot
tell this caller apart from `CrossMerge.Use.caller`, and the project's version of
`shared` is a `sorry`, so it reads `verified` too. -/
