...
4. **Verify** -- parses sorry warnings from build output to determine verification status; axioms and non-theorem `*External.lean` declarations are marked `"trusted"` with a `trusted-reason` (`"axiom"` or `"external"`) for trust-base classification; theorems in `*External.lean` carry real proofs and receive their normal verification status; declarations without source location (kernel-synthesized) are filtered from output (skippable via `--skip-verify`)
