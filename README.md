...
2. **Atomize** -- walks the Lean environment, extracts declarations with type and term dependencies, then **folds auxiliary edges**: Lean abstracts non-atomic embedded proofs and match arms into constants probe-lean does not emit (`X._proof_N`, `X.match_N`, …), and a dependency reached only through one of those used to vanish from the graph entirely. Such edges are now recovered into the referencing declaration. The pass is strictly additive — it only adds project names to `type-dependencies`/`term-dependencies`, never removes an entry, never touches the `*-external` arrays, and never changes the atom set. Structural members (`.mk`, `.injEq`, `.casesOn`, …) are deliberately not folded through, and only project-internal targets are recovered, which makes the external arrays abstraction-sensitive (`host → anchor` is listed, `host → aux → anchor` is not). See [docs/SCHEMA.md](docs/SCHEMA.md#auxiliary-dependency-folding) for the full contract and its limits — in particular, folding fixes *edges*, not `verification-status` soundness, and a zero in-degree is still not a licence to delete a declaration
