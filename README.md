...
3. **Filter** -- applies config-driven flags from `.verilib/probes/config.json` (`is-hidden`, `is-aeneas-generated`, `is-ignored`) and auto-detects Lean-generated code (`deriving`-generated instance clusters and structure/class projections), flagging it `is-lean-generated` + `is-hidden` so `viewify` omits it. Generated atoms are **kept in the dependency graph** (so transitive-verification stays sound), only hidden from the presented view. After enrichment, `is-hidden` is cleared on *contaminated* lean-generated atoms (locally verified but not `transitively-verified`, or `unverified`/`failed`) in the `extract` output, so consumers that read it directly (e.g. the web UI) can surface them for tracing; `viewify` molecules still omit all generated atoms regardless of `is-hidden`
