...
3. **Filter** -- applies config-driven flags from `.verilib/probes/config.json` (`is-hidden`, `is-aeneas-generated`, `is-ignored`) and auto-detects Lean-generated code (`deriving`-generated instance clusters, structure/class projections, and attribute-macro companion theorems such as the `X.mvcgen_spec` that Aeneas's `@[step]` adds next to a tagged `theorem X`; companions of tagged *axioms* stay visible as the axiom's spec proxy), flagging it `is-lean-generated` + `is-hidden` so `viewify` omits it. Lean-generated theorems are also excluded from `specs` lists and the heuristic primary-spec signals; an explicit `@[primary_spec]` still wins. Generated atoms are **kept in the dependency graph** (so transitive-verification stays sound), only hidden from the presented view. After enrichment, `is-hidden` is cleared on *contaminated* lean-generated atoms (locally verified but not `transitively-verified`, or `unverified`/`failed`) in the `extract` output, so consumers that read it directly (e.g. the web UI) can surface them for tracing; `viewify` molecules still omit all generated atoms regardless of `is-hidden`
