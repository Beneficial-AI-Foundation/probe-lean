...
| `Audit5.lean` | Type-vs-value split: are the hidden auxiliaries in a declaration's type or its body? Measured against the pre-decision design, in which type-position folding changed `typeDependencies` (which `computeSpecs` walks) and so had a `specs`/`primary-spec` blast radius; the shipped fold routes type-position finds to `term-dependencies` instead, see `docs/auxiliary-folding.md`. |
...
| `check-status-consistency.py` | **Artifact ⇔ `check-axioms` gate.** Both come from the same kernel walk, so they must agree exactly on every emitted atom, in both directions: listed `[direct]` ⇔ `unverified`, listed ⇔ `verified`, not listed ⇔ `transitively-verified` or `trusted`, and every listed name without `[not emitted]` is an atom. Run in CI on the fixtures; `--allow-missing` for `--skip-verify` artifacts, `--no-upgrade` for `--skip-enrich` ones. |
