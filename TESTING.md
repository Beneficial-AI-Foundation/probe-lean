...
| `testTrustListingFormat` | `check-axioms` T listing text: `formatTrustHeader`, `formatTrustedLine` (an axiom without a type, a rule-3 entry with its statement), the `Note(axiom):` generated-axiom line |
